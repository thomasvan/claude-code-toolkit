#!/usr/bin/env python3
"""
Blind A/B test: CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1 vs 0.

Variant A — adaptive thinking DISABLED (fixed high reasoning budget per turn)
Variant B — adaptive thinking ENABLED  (model dynamically adjusts budget)

10 rounds, 20 total invocations. Each pair runs in parallel.
Outputs are saved to results/{round_num}_{hex_id}.txt.
Mapping (hex_id -> variant) is saved to mapping.json — keep away from scorer.
"""
...
PROMPT_TEMPLATE = """\
Review the following Python file for bugs, security issues, and improvements. For each finding, provide:
- Severity: CRITICAL / HIGH / MEDIUM / LOW
- Line number(s)
- Description of the issue
- Suggested fix

Be thorough. Do not fabricate issues — only report real problems you can point to in the code.

File: hooks/pretool-unified-gate.py
...
def random_hex_id() -> str:
    """Return a random 4-char hex string."""
    return f"{random.randint(0, 0xFFFF):04x}"


def unique_hex_ids(count: int) -> list[str]:
    """Generate `count` unique hex IDs."""
    ids: set[str] = set()
    while len(ids) < count:
        ids.add(random_hex_id())
    return list(ids)


def run_claude(hex_id: str, disable_adaptive: bool, round_num: int) -> dict:
    """Run a single claude -p invocation.

    Args:
        hex_id: Blind label for this run.
        disable_adaptive: True -> CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1
        round_num: Round number for logging.

    Returns:
        Dict with hex_id, variant, exit_code, duration_s, output_file, error.
    """
    variant = "A" if disable_adaptive else "B"
    env = os.environ.copy()
    env["CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING"] = "1" if disable_adaptive else "0"

    output_file = RESULTS_DIR / f"{round_num:02d}_{hex_id}.txt"
    ...
    print(f"  [round {round_num:02d}] starting {hex_id} (variant={variant})", flush=True)
    ...
    # Write output file regardless of success/failure
    with open(output_file, "w", encoding="utf-8") as fh:
        fh.write(f"hex_id: {hex_id}\n")
        fh.write(f"round: {round_num}\n")
        fh.write(f"exit_code: {exit_code}\n")
        fh.write(f"duration_s: {duration:.2f}\n")
        fh.write(f"disable_adaptive_thinking: {disable_adaptive}\n")
        fh.write("--- output ---\n")
    ...
    print(f"  [round {round_num:02d}] {hex_id} done in {duration:.1f}s — {status}", flush=True)
    ...
    return {
        ...
        "output_file": str(output_file.relative_to(SCRIPT_DIR)),
        "error": error_text if exit_code != 0 else "",
    }
...
def main() -> None:
    """Run all rounds and save mapping."""
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    # Generate 2 * ROUNDS unique IDs upfront so the scorer never sees which
    # ID corresponds to which variant until generate_report.py loads mapping.json.
    all_ids = unique_hex_ids(ROUNDS * 2)
    ...
    print(f"Starting adaptive-thinking A/B test: {ROUNDS} rounds, 2 variants")
    print(f"Results dir: {RESULTS_DIR}")
    print(f"Mapping file: {MAPPING_FILE} (do not share with scorer)\n")

    for round_num in range(1, ROUNDS + 1):
        id_a = all_ids[(round_num - 1) * 2]
        id_b = all_ids[(round_num - 1) * 2 + 1]

        mapping[id_a] = {"variant": "A", "disable_adaptive": True, "round": round_num}
        mapping[id_b] = {"variant": "B", "disable_adaptive": False, "round": round_num}

        print(f"\nRound {round_num}/{ROUNDS} — running A and B in parallel")

        with ThreadPoolExecutor(max_workers=2) as pool:
            future_a = pool.submit(run_claude, id_a, True, round_num)
            future_b = pool.submit(run_claude, id_b, False, round_num)
            for future in as_completed([future_a, future_b]):
                run_log.append(future.result())

    # Save mapping — scorer must NOT read this file
    with open(MAPPING_FILE, "w", encoding="utf-8") as fh:
        json.dump(mapping, fh, indent=2)

    # Save run log for diagnostics
    run_log_file = SCRIPT_DIR / "run_log.json"
    with open(run_log_file, "w", encoding="utf-8") as fh:
        json.dump(run_log, fh, indent=2)

    failures = [r for r in run_log if r["exit_code"] != 0]
    print(f"\nDone. {len(run_log)} runs completed, {len(failures)} failures.")
    if failures:
        print("Failed runs:")
        for f in failures:
            print(f"  round={f['round']} hex_id={f['hex_id']} variant={f['variant']} error={f['error'][:80]}")
    print(f"\nNext: run score_results.py to score all outputs blindly.")
